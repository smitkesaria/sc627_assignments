...
class Vector:
    ...
    def __add__(self, v):
        """Create custom add implementation."""
        return Vector(self.x + v.x, self.y + v.y)

    def __sub__(self, v):
        """Create custom sub implementation."""
        return Vector(self.x - v.x, self.y - v.y)
...
def computeBug1(
    ...
    while current_position.get_distance(goal) > step_size:
        ...
        if "Failure" in success:
            ...
            starting_closest_point_on_polygon = computeClosestPointOnPolygon(
            ...
            while True:
                ...
                current_closest_point_on_polygon = computeClosestPointOnPolygon(
                    closest_polygon, current_position
                )
                ...
                if (
                    abs(
                        current_closest_point_on_polygon.x
                        - starting_closest_point_on_polygon.x
                    )
                    < step_size / 2
                    and abs(
                        current_closest_point_on_polygon.y
                        - starting_closest_point_on_polygon.y
                    )
                    < step_size / 2
                ):
                    break
...
def computePotentialPlanner(
    start: Point,
    goal: Point,
    obstaclesList: Tuple[Polygon, ...],
    step_size: float,
) -> Tuple[Point, ...]:
    """."""
    current_position = start
    path = []
    while current_position.get_distance(goal) > step_size:
        direction = computePotentialGradient(current_position, goal, obstaclesList)
        current_position = current_position.move_magnitude_towards(direction, step_size)
        path.append(current_position)
        # print(f"{current_position},")
        if len(path) > 3 and path[-1] == path[-3]:
            print("Exiting, as it seems to be oscillating.")
            break
    return path


def computePotentialGradient(
    current: Point,
    goal: Point,
    obstaclesList: Tuple[Polygon, ...],
) -> Vector:
    def computeAttractiveGradient(current: Point, goal: Point) -> Vector:
        """."""
        chi = 0.8
        d_star_goal = 2
        dist_to_goal = current.get_distance(goal)
        if dist_to_goal <= d_star_goal:
            dir_x = (1 / 2) * chi * 2 * (current.x - goal.x)
            dir_y = (1 / 2) * chi * 2 * (current.y - goal.y)
        else:
            dir_x = chi * d_star_goal * (1 / 2) * (current.x - goal.x) / dist_to_goal
            dir_y = chi * d_star_goal * (1 / 2) * (current.y - goal.y) / dist_to_goal
        return Vector(dir_x, dir_y)

    def computeRepulsiveGradient(
        current: Point,
        obstaclesList: Tuple[Polygon, ...],
    ) -> Vector:
        """."""
        ...
        fin_dir = Vector(0, 0)
        for obstacle in obstaclesList:
            dist_to_obstacle = computeDistancePointToPolygon(obstacle, current)
            closest_point_on_obstacle = computeClosestPointOnPolygon(obstacle, current)
            if dist_to_obstacle <= q_start_obstacle:
                dir_x = (
                    2
                    * (current.x - closest_point_on_obstacle.x)
                    * ((1 / dist_to_obstacle) - (1 / 2))
                    / (dist_to_obstacle) ** 3
                )
                dir_y = (
                    2
                    * (current.y - closest_point_on_obstacle.y)
                    * ((1 / dist_to_obstacle) - (1 / 2))
                    / (dist_to_obstacle) ** 3
                )
                fin_dir += Vector(dir_x, dir_y)

        return fin_dir

    return (
        Vector(0, 0)
        - computeAttractiveGradient(current, goal)
        + computeRepulsiveGradient(current, obstaclesList)
    )
